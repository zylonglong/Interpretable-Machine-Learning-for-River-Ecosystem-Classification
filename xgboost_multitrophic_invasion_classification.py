...
# ============================================================
# XGBoost Optimized Full Pipeline (data-only version)
# - Keeps the SAME research pipeline structure:
#   1) Outer loop: 5 repeated stratified splits (80/20) with seeds 42..46
#   2) Inner loop: RandomizedSearchCV + 5-fold StratifiedKFold (tuning)
#   3) Evaluation: export confusion matrix + multi-class ROC (OvR) + reports + predictions
#   4) Learning curve: fixed validation via StratifiedShuffleSplit, mean±std + raw export
#   5) SHAP: TreeExplainer, save long-format beeswarm/dependence data, feature importance, JSON
#   6) Aggregation: merge 5 folds, compute mean±sd, export aggregated files
# - All plotting steps removed; only plotting data retained
# ============================================================
...
def build_xgb_base_model(n_classes: int):
...
def tune_hyperparameters(X_train, y_train, n_classes: int):
    ...
    base_model = build_xgb_base_model(n_classes)
    ...
    print("\n[XGBoost] 开始 RandomizedSearchCV 超参数调优 ...")
    rs.fit(X_train, y_train)
    print("[XGBoost] 调参完成。best_score =", rs.best_score_)
...
def plot_learning_curve(model_params: dict, X_train, y_train, fold_dir_path: str, n_classes: int):
    ...
    records = []
    for frac in tqdm(train_fracs, desc="[XGBoost] Learning curve"):
        fold_counter = 0
        for tr_idx, val_idx in splitter.split(X_train, y_train):
            ...
            mdl.set_params(objective="multi:softprob", num_class=n_classes, eval_metric="mlogloss")
...
def aggregate_shap_results(base_target_out: str):
    ...
    if not beeswarm_all or not imp_all:
        print("[XGBoost] aggregate_shap_results: 未找到 fold SHAP 输出，跳过聚合。")
        return None
...
def run_target_pipeline(target_column: str):
    print("\n" + "=" * 60)
    print(f"[XGBoost] 开始分析目标变量: {target_column}")
    print("=" * 60)
    ...
    for fold_id, seed in enumerate(CFG.outer_seeds, start=1):
        print(f"\n[XGBoost] ===== Fold {fold_id} / Seed {seed} =====")
        fm = run_single_fold(
            X=X, y=y, le=le,
    ...
    print(f"\n[XGBoost] 目标变量 {target_column} 全流程完成。输出目录：{base_out}")
    return summary
...
def main():
    ...
    print("\n" + "=" * 60)
    print(f"[XGBoost] 所有目标变量完成！综合结果已保存：{final_path}")
    print("=" * 60)
